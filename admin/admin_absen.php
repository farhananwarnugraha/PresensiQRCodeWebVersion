<?php
  ...
  $absen = query ("SELECT presensi_masuk.id_presensi, presensi_masuk.siswa,presensi_masuk.waktu_presensi,presensi_masuk.tanggal_presensi,presensi_masuk.keterangan,siswa.id_siswa,siswa.nis_siswa,siswa.nama_siswa,siswa.kelas_siswa,siswa.jurusan_siswa,kelas.id_kelas,kelas.nama_kelas,jurusan.id_jurusan,jurusan.nama_jurusan FROM `presensi_masuk` JOIN siswa ON siswa.id_siswa = presensi_masuk.siswa JOIN kelas ON siswa.kelas_siswa = kelas.id_kelas JOIN jurusan ON siswa.jurusan_siswa = jurusan.id_jurusan ORDER BY presensi_masuk.tanggal_presensi ASC");
  $absen_mapel = query("SELECT * FROM presensi_mapel");
  ...
  <body>
    ...
    <main class="mt-5 shadow pb-1 mb-5 bg-body rounded">
      <div class="container-fluid">
        ...
        <!-- menekan button input absen -->
        <?php
          if( isset($_POST['submit'] )){

            if (absen($_POST) >0) {
              // var_dump(absen($_POST['tgl_absen']));
              // exit;
              echo "
                <script>
            ...
            <div class="modal-content">
              ...
              <div class="modal-body">
                ...
                <form action=" " method="POST">
                  ...
                  <div class="mb-3">
                    <label for="nama_mapel" class="form-label">Tanggal Presensi </label>
                    <input type="date" class="form-control" id="nama_mapel" name="tgl_absen" aria-describedby="namamapel" required>
                  </div>
                  <div class="mb-3">
                    <label for="nama_mapel" class="form-label">Waktu Presensi </label>
                    <input type="time" class="form-control" id="nama_mapel" name="jam_absen" aria-describedby="namamapel" required>
                  </div>
                  <div class="mb-3">
          ...
          <thead class="text-center align-middle">
            ...
            ?>
              <tr>
                ...
                <td class="text-center"><?= $row['tanggal_presensi']?></td>
                <td class="text-center"><?= $row['waktu_presensi']?></td>
                <td class="text-center"><?= $row['keterangan']?></td>
                <td class="text-center">
                  <a href="editabsenmasuk.php?id=<?= $row['id_presensi']?>" type="button" class="btn btn-warning btn-sm mb-2">
                    <i class="bi bi-pencil-square"></i>
                    Ubah
                  </a>
                    <a href="hapus_absen.php?id=<?= $row['id_presensi']?>"  onclick="return confirm('Apakah Anda ingin menghapus data ini ?')" type="button" class="btn btn-danger btn-sm">
